#!/usr/bin/env python3
"""runner_dl2: 云端快速版——短退避（Azure IP 对 archive.org 的 403 是间歇性的，
长退避得不偿失），4s 间隔。用法同 runner_dl.py。"""
import json
import re
import sys
import time
    ...
    for attempt in range(RETRIES):
        try:
            with urllib.request.urlopen(req, timeout=90) as resp:
                if resp.status == 200:
                    return resp.read(MAX_BYTES + 1), resp.headers.get("Content-Length")
        except urllib.error.HTTPError as e:
            if e.code in (403, 429):
                w = BACKOFFS[min(attempt, len(BACKOFFS) - 1)]
                print(f"  [{e.code}] 退避 {w}s", flush=True)
                time.sleep(w)
                continue
...
def main() -> None:
    ...
    for i, rec in enumerate(todo, 1):
        ...
        if ok:
            ...
            print(f"  [{i}/{len(todo)}] ✅ {year}/{name[:45]} ({len(data)}B)", flush=True)
        else:
            failed.append(url)
            manifest.append({"url": url, "status": "failed", "reason": why})
            print(f"  [{i}/{len(todo)}] ❌ {name[:45]} ({why})", flush=True)
